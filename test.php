...
function get_permutations($inputcolor, $count) {
    ...
    foreach ($temp1 as $key => $value) {
        $acval = explode(',', $value);
        $acval = implode(",", array_slice($acval, 0, $count));
        $query = "(select nc.nfw_product_id, group_concat(nc.nfw_color_id order by FIELD(nc.nfw_color_id, $acval)) colorbrc from nfw_product_color as nc 
where nc.nfw_color_id in ($acval)
group by nc.nfw_product_id 
having count(nc.nfw_product_id) = $count 
order by  colorbrc  desc) ";
        ...
        if (count($result)) {
         ...
         having count(nfw_product_id) = " . $colorcount . " order by FIELD(nfw_color_id, " . $colorquery . ")";
                ...
                if ($colorcount > 1) {
                    ...
                    $queryc1a = get_permutations($colortemparray, 2);
                    ...
                    $queryc1b = "
SELECT nfw_product_id, nfw_color_id FROM nfw_product_color
      where nfw_color_id in (" . $colorquery . ") and nfw_product_id in (select nfw_product_id from nfw_product_color group by nfw_product_id having count(nfw_product_id) = 1 )
      group by nfw_product_id having count(nfw_product_id) = 1 
