...
//                sub category checking string
     ...
     where nfw_product_tag_connection.tag_id = '$item_type' and nfw_product_subcategory.category_id in ($categoryString )";
        ...
        $query = " select id, color, title, product_speciality, price, price_r, sale_price, image1, image2, sort_type, publishing from (" . $query . "  )  as dc where publishing = 1
                            group by id order by  $pricesort, colorid desc $limitquery";
        ...
        if (count($result)) {
         ...
         having count(nfw_product_id) = " . $colorcount . " order by FIELD(nfw_color_id, " . $colorquery . ")";
                ...
                if ($colorcount > 1) {
                    ...
                    $queryc1a = "SELECT nfw_product_id, nfw_color_id FROM nfw_product_color
      where nfw_color_id in (" . $colorquery . ") and nfw_product_id in (select nfw_product_id from nfw_product_color group by nfw_product_id having count(nfw_product_id) = " . $colorcount . " or nfw_color_id in (" . $colorquery . ") )
      group by nfw_product_id having count(nfw_product_id) = " . $colorcount . " 
          order by FIELD(nfw_color_id, " . $colorquery . ")";


                    $queryc1a1 = "select nfw_product_id, nfw_color_id, group_concat(nfw_color_id)
 from nfw_product_color where  nfw_color_id in (" . $colorquery . ") 
group by nfw_product_id 
having count(nfw_color_id) <=3
order by count(nfw_color_id) desc, FIELD(nfw_color_id, " . $colorquery . ") ";

                    $colorproductmainlist1a = resultAssociate($queryc1a);
                    $temp41a = [];
                    foreach ($colorproductmainlist1a as $key11 => $value11) {
                        array_push($temp41a, $value11['nfw_product_id']);
                    }
                    
                    

                    $colorproductmainlist1b = resultAssociate($queryc1b);
                    $temp41b = [];
                    foreach ($colorproductmainlist1b as $key11 => $value11) {
                        array_push($temp41b, $value11['nfw_product_id']);
                    }

                    $temp41 = array_unique(array_merge($temp41a, $temp41b));
                } else {
                    $queryc1 = "SELECT nfw_product_id, nfw_color_id  FROM nfw_product_color 
   where nfw_product_id in (" . $productquery . ") 
    group by nfw_product_id 
    having  nfw_color_id in (" . $colorquery . ") 
order by count(nfw_color_id) asc";
...
class CartHandler {
    ...
    function insertInOrderTable($quan, $arry, $user_id, $bill_id, $ship_id, $coupon_id, $card_id, $skus, $imagess, $prices, $tag_titles, $wallet, $ship_amt, $subtotal) {
        ...
        if ($card_id == 'paypal') {
            ...
            $carddata1 = "Post Pay To Admin";
            $cardTitle = 'Post Pay To Admin';
        } else {
...
//        }
        ...
        //echo '6';
        mysql_query("INSERT INTO `nfw_order_payment` (`user_id`, `order_id`, `card_id`, `transaction_no`, `transaction_amount`,`status`) VALUES ('$user_id','$last_id','$card_id','0','$total_sum','Awaited')");
        //echo '7';
